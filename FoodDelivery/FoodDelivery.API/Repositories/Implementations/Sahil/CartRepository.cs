...
{
    /// <summary>
    /// Cart is stored in HTTP Session (server-side) as JSON.
    /// Key format: "Cart_{cartId}"
    /// </summary>
    public class CartRepository : ICartRepository
    {
        ...
        private async Task RecalculateCart(CartDto cart)
        {
            ...
            cart.DeliveryFee = cart.Subtotal >= 500 ? 0 : 40;           // Free above ₹500
            var taxable      = cart.Subtotal - cart.DiscountAmount;
            cart.TaxAmount   = Math.Max(0, taxable) * 5 / 100;          // 5% GST
